...
def analyze_polysemy(df_test, pred_col, poly_words, max_examples=3):
    ...
    for word in poly_words:
        def count_in(sub_df, w=word):
            return int(sub_df["text"].apply(lambda t: word_is_present(w, t)).astype(bool).sum())
...
def write_report(
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"Eval set  : {eval_path}  ({n_total} rows, "
                f"{n_slang} SLANG / {n_not} NOT_SLANG)\n")
        f.write(f"Models    : Dictionary Baseline, TF-IDF Baseline\n\n")

        # ── Section 1: Metrics ────────────────────────────────────────────────
        f.write(SEP + "\n")
        f.write(f"SECTION 1: MODEL METRICS SUMMARY ({split_label} SET)\n")
        f.write(SEP + "\n\n")
...
def run_for_split(df_eval, eval_path, split_label, slang_list, pipeline, output_path, top_n, max_ex):
    df = df_eval.copy()

    print(f"Running Dictionary model on {split_label} set...")
    _, dict_pred_labels = make_predictions(df["text"].tolist(), slang_list)
    df["dict_pred"] = dict_pred_labels

    print(f"Running TF-IDF model on {split_label} set...")
    df["tfidf_pred"] = pipeline.predict(df["text"].fillna("").astype(str)).tolist()

    label_map = {SLANG: 1, NOT_SLANG: 0}
    y_true = df["label"].map(label_map).tolist()

    dict_metrics  = compute_metrics(y_true, df["dict_pred"].map(label_map).tolist())
    tfidf_metrics = compute_metrics(y_true, df["tfidf_pred"].map(label_map).tolist())
    ...
    dict_errors  = get_error_buckets(df, "dict_pred")
    tfidf_errors = get_error_buckets(df, "tfidf_pred")
    _, _, fp_d, fn_d = dict_errors
    _, _, fp_t, fn_t = tfidf_errors
    print(f"  Dict  FP={len(fp_d)}  FN={len(fn_d)}")
    print(f"  TF-IDF FP={len(fp_t)}  FN={len(fn_t)}")

    dict_poly  = analyze_polysemy(df, "dict_pred",  POLYSEMY_WORDS, max_ex)
    tfidf_poly = analyze_polysemy(df, "tfidf_pred", POLYSEMY_WORDS, max_ex)
...
def main():
    ...
    df_gen     = pd.read_csv(args.generalization)
    ...
    print("\n=== TEST SET ===")
    run_for_split(
        df_test, args.test, "TEST", slang_list, pipeline,
        os.path.join(args.output_dir, "baseline_test_report.txt"),
        args.top_n, args.max_ex,
    )

    print("\n=== GENERALIZATION TEST SET ===")
    run_for_split(
        df_gen, args.generalization, "GENERALIZATION TEST", slang_list, pipeline,
        os.path.join(args.output_dir, "baseline_generalization_report.txt"),
        args.top_n, args.max_ex,
    )
